...
class SmartVideoExtractor:
    ...
    def extract_video_intelligently(self,
                                   video_path: str,
                                   output_dir: str,
                                   num_frames: int = 225,
                                   dish_to_class: Optional[Dict[int, str]] = None,
                                   detection_frame: int = 0,
                                   max_larvae_check_frames: int = 500) -> int:
        """
        ...
        larvae_start_frames = self.larvae_detector.detect_larvae_appearance(
        ...
        for dish_idx, (dish_roi, start_frame) in enumerate(
            ...
            seq_dir = os.path.join(output_dir, class_name, 
            ...
            if extracted_count > 0:
                sequences_created += 1
                logger.info(f"Created sequence for dish {dish_idx} "
    ...
    def _extract_frames_from_indices(self,
                                   video_path: str,
    ...
    def analyze_video_quality(self, video_path: str) -> Dict:
        ...
        sample_frames = np.linspace(0, total_frames-1, min(50, total_frames), 
                                   dtype=int)
        ...
        try:
            dish_rois = detect_dishes_in_video(video_path, 0, 
...
def extract_video_with_smart_detection(
    ...
    num_frames: int = 225,
    dish_to_class: Optional[Dict[int, str]] = None,
    use_auto_detection: bool = True) -> int:
    """
