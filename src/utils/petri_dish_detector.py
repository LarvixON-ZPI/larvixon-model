...
class PetriDishDetector:
    ...
    def detect_edges(self, gray_frame: np.ndarray) -> np.ndarray:
        ...
        - Binary edge image
        """
        # Apply adaptive threshold for better edge detection under varying
        # lighting
        ...
        # Combine adaptive threshold and Canny edges
    ...
    def filter_contours(self, contours) -> List[np.ndarray]:
        ...
        for contour in contours:
            ...
            # Should have roughly 4-8 vertices for a rounded rectangle
    ...
    def validate_dish_appearance(self, frame: np.ndarray, 
        ...
        # Check for circular/rounded patterns using template matching or
        # other methods. For now, we'll use basic statistics
        
        # Calculate standard deviation of pixel values
        # Petri dishes usually have relatively uniform appearance
        std_dev = float(np.std(gray_roi.astype(np.float32)))
        
        # Check if the ROI has reasonable contrast
        min_val = np.min(gray_roi)
        max_val = np.max(gray_roi)
        contrast = max_val - min_val
        
        # Basic validation criteria
        if std_dev < 10:  # Too uniform, might be empty space
            return False
    ...
    def detect_dishes(self, frame: np.ndarray, 
        ...
        for contour in valid_contours:
            ...
            # Add some padding around the detected dish
            padding = 10
            x = max(0, x - padding)
            y = max(0, y - padding)
            ...
            # Validate appearance if requested
    ...
    def remove_overlapping_detections(
        ...
        # Find non-overlapping detections
        keep = []
