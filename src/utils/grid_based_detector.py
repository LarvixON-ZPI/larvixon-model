...
class GridBasedPetriDetector:
    ...
    def _validate_roi_content(self, frame: np.ndarray, 
...
class AdaptiveGridDetector:
    ...
    def _score_detections(self, frame: np.ndarray, 
        ...
        for roi in detections:
            ...
            aspect_ratio = w / h
            ar_score = 1.0 - abs(aspect_ratio - 1.0)  # Closer to 1.0 is better
...
def detect_dishes_with_grid(frame: np.ndarray, 
                           save_visualization: bool = True,
                           output_path: str = "grid_detection_result.jpg") -> List[Tuple[int, int, int, int]]:
    """
    ...
    if save_visualization:
        ...
        for i, (x, y, w, h) in enumerate(detections):
            ...
            cv2.putText(vis_frame, f"Dish {i+1}", (x, y - 10),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        
        # Add configuration info
        cv2.putText(vis_frame, f"Grid: {config_name}", (10, 30),
...
def detect_dishes_in_video_grid(video_path: str, 
                               frame_index: int = 0,
